...
\section{Installation with CMake}

As an experimental feature, \charmpp{} can be installed with the CMake tool, version~3.8 or newer.
This is currently only supported on Linux and Darwin, but not on Windows.

After downloading and unpacking \charmpp{}, it can be installed in the following way:
\begin{alltt}
    $ cd charm
    $ mkdir build
    $ cd build
...
By default, CMake builds the netlrts version with SMP enabled. Other configuration options can be specified in the cmake command above. For example, to build the MPI version of \charmpp{} with the SMP mode disabled, the following command can be used:
